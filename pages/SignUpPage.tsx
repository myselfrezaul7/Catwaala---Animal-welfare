...
const SignUpPage: React.FC = () => {
  ...
  }, [name, email, password, signup, navigate, t]);
  ...
  return (
    <div className="container mx-auto px-6 py-12 flex-grow flex items-center justify-center">
      <div className="w-full max-w-md bg-slate-100/30 dark:bg-slate-800/30 backdrop-blur-lg border border-white/20 dark:border-slate-700/50 p-8 md:p-10 rounded-2xl shadow-xl">
        <h1 className="text-3xl font-bold text-center text-slate-800 dark:text-slate-100 mb-6">{t('signup.title')}</h1>
        
        {error && <Alert type="error" title={t('signup.errorTitle')} message={error} className="mb-4" />}
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center">
              ...
              <span className="flex-shrink mx-4 text-slate-500 dark:text-slate-400 text-sm">{t('signup.orSeparator')}</span>
              <div className="flex-grow border-t border-slate-300 dark:border-slate-600"></div>
          </div>

          <form onSubmit={handleSubmit} className="space-y-6">
            <div>
              <label htmlFor="name" className="block text-base font-semibold text-slate-700 dark:text-slate-300 mb-2">{t('signup.nameLabel')}</label>
              <input 
                type="text" 
                id="name" 
                value={name}
            ...
            <div>
              <label htmlFor="email" className="block text-base font-semibold text-slate-700 dark:text-slate-300 mb-2">{t('signup.emailLabel')}</label>
              <input 
                type="email" 
                id="email" 
                value={email}
            ...
            <div>
              <label htmlFor="password" className="block text-base font-semibold text-slate-700 dark:text-slate-300 mb-2">{t('signup.passwordLabel')}</label>
              <input 
                type="password" 
                id="password" 
                value={password}
         ...
         <p className="text-center mt-6 text-slate-600 dark:text-slate-400">
          {t('signup.hasAccount')} <Link to="/login" className="font-bold text-orange-600 dark:text-orange-400 hover:underline">{t('signup.loginLink')}</Link>
        </p>
      </div>
    </div>
  );
