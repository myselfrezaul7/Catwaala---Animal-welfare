    ...
    <>
        <div className="container mx-auto px-6 py-16">
            <div className="text-center mb-16">
                <h1 className="text-4xl md:text-5xl font-bold text-slate-800 dark:text-slate-100">{t('ourImpact.title')}</h1>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                <StatCard value={rescuedThisYear} label={t('ourImpact.stats.rescued')} icon={<CatIcon className="w-12 h-12" />} />
                <StatCard value={successfulAdoptions} label={t('ourImpact.stats.adoptions')} icon={<HeartIcon className="w-12 h-12" />} />
                <StatCard value={mealsProvided} label={t('ourImpact.stats.meals')} icon={<CatIcon className="w-12 h-12" />} />
            </div>
            ...
            <div className="mt-16 text-center">
                ...
                >
                    <h3 className="text-2xl font-bold text-slate-800 dark:text-slate-100">{t('ourImpact.happyTails.title')}</h3>
                    <p className="text-slate-600 dark:text-slate-400 mt-1">{t('ourImpact.happyTails.subtitle')}</p>
                </Link>
            </div>
            ...
            <div className="mt-16 bg-slate-100/30 dark:bg-slate-800/30 backdrop-blur-lg border border-white/20 dark:border-slate-700/50 rounded-2xl shadow-xl p-8 md:p-12 text-center">
                <h2 className="text-3xl font-bold text-slate-800 dark:text-slate-100 mb-4">{t('ourImpact.shelter.title')}</h2>
