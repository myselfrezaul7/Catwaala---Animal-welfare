      ...
      <div className="bg-slate-100/70 dark:bg-slate-800/70 backdrop-blur-lg border border-white/20 dark:border-slate-700/50 rounded-2xl shadow-2xl w-full max-w-lg" onClick={(e) => e.stopPropagation()}>
        <div className="p-8">
          <div className="flex justify-between items-start mb-4">
            <div>
                <h2 className="text-3xl font-bold text-slate-800 dark:text-slate-100">Book Appointment</h2>
                <p className="text-slate-600 dark:text-slate-300 text-lg mt-1">with <span className="font-bold text-slate-800 dark:text-slate-100">{vet.name}</span></p>
            </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                  <label htmlFor="petName" className={labelStyles}>Pet's Name</label>
                  <input type="text" id="petName" required className={inputStyles} />
              </div>
              <div>
                  <label htmlFor="reason" className={labelStyles}>Reason for Consultation</label>
                  <textarea id="reason" rows={3} required placeholder="e.g., Skin rash, unusual behavior..." className={inputStyles}></textarea>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <label htmlFor="date" className={labelStyles}>Preferred Date</label>
                    <input type="date" id="date" required className={inputStyles} />
                </div>
                <div>
                    <label htmlFor="time" className={labelStyles}>Preferred Time</label>
                    <input type="time" id="time" required className={inputStyles} />
                </div>
              </div>

            <div className="pt-5 border-t border-slate-300 dark:border-slate-700 mt-2">
