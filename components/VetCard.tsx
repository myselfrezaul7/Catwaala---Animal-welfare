...
const VetCard: React.FC<VetCardProps> = ({ vet, onBookAppointment }) => {
  return (
    <div className="bg-slate-100/30 dark:bg-slate-800/30 backdrop-blur-lg border border-white/20 dark:border-slate-700/50 rounded-xl shadow-xl overflow-hidden flex flex-col text-center items-center p-6 transform hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 ease-in-out">
      ...
      <h3 className="text-xl font-bold text-slate-800 dark:text-slate-100 mt-4">{vet.name}</h3>
      <p className="text-slate-600 dark:text-slate-400 font-medium flex-grow">{vet.specialization}</p>
      <p className={`mt-2 font-semibold text-sm ${vet.isOnline ? 'text-green-600 dark:text-green-400' : 'text-gray-500 dark:text-gray-400'}`}>
